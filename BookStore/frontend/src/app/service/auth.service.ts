...
export class AuthService {
  private API_URL = 'http://https://book-store-3-svnz.onrender.com//auth';
  ...
  checkEmailExists(email: string): Observable<boolean> {
    return this.http.get<{ exists: boolean }>(`http://https://book-store-3-svnz.onrender.com//auth/check-email?email=${email}`)
  ...
  requestPasswordReset(email: string) {
    return this.http.post<{ message: string; resetToken?: string }>(
      'http://https://book-store-3-svnz.onrender.com//auth/forgot-password', // hoặc env
      { email }
    );
  }
